...
intérprete para mostrar la salida real del programa:
  ...
  (extra) Reasignación    -> una variable ya declarada puede recibir un
                             nuevo valor con 'nombre = expresion;'
    ...
    while i < n:
        ...
        if c == '"':
            ...
            while j < n and source[j] != '"' and source[j] != '\n':
                s += source[j]; j += 1
            if j >= n or source[j] != '"':
                errors.append({'line': line, 'msg': f'Error léxico: Cadena de texto sin cerrar: "{s}'})
                i = j; continue
        ...
        if is_digit(c):
            ...
            if j < n and source[j] == '.' and j + 1 < n and is_digit(source[j + 1]):
                ...
                while j < n and is_digit(source[j]):
                    num += source[j]; j += 1
            tokens.append({'type': 'NUMERO_DECIMAL' if is_float else 'NUMERO_ENTERO', 'lexema': num, 'line': line})
            i = j; continue
        if is_alpha(c):
            j = i
            word = ''
        ...
        two = source[i:i + 2]
        if two in ('>=', '<=', '==', '!=', '&&', '||'):
            tokens.append({'type': 'OPERADOR', 'lexema': two, 'line': line}); i += 2; continue
        if c in '+-*/><':
            tokens.append({'type': 'OPERADOR', 'lexema': c, 'line': line}); i += 1; continue
        if c == '=':
            tokens.append({'type': 'OPERADOR_ASIGNACION', 'lexema': c, 'line': line}); i += 1; continue
        if c in ';{}()':
            names = {';': 'PUNTO_Y_COMA', '{': 'LLAVE_ABRE', '}': 'LLAVE_CIERRA', '(': 'PARENTESIS_ABRE', ')': 'PARENTESIS_CIERRA'}
            tokens.append({'type': names[c], 'lexema': c, 'line': line}); i += 1; continue
        errors.append({'line': line, 'msg': f"Error léxico: Carácter no reconocido: '{c}'"})
        i += 1
...
# ============================================================
# 2 + 3 + 4. PARSER + TABLA DE SÍMBOLOS + VALIDACIÓN DE TIPOS + EJECUCIÓN
#
# Gramática:
#   programa      -> sentencia* FIN
#   sentencia     -> declaracion | asignacion | condicional | imprimirStmt
...
#   expresion     -> termino (('+'|'-'|comparador) termino)*
...
def parse_and_analyze(tokens):
    ...
    def peek_next():
        return tokens[pos + 1] if pos + 1 < len(tokens) else tokens[-1]
    ...
    def error(msg):
        errors.append({'line': peek()['line'], 'msg': f"Error sintáctico: {msg}"})
        raise ParseError()
    ...
    def factor():
        ...
        if t['type'] == 'IDENTIFICADOR':
            ...
            if info is None:
                errors.append({'line': t['line'], 'msg': f"Error semántico: variable no declarada: '{t['lexema']}'"})
    ...
    def declaracion():
        ...
        if id_tok['lexema'] in scope_stack[-1]:
            errors.append({'line': id_tok['line'], 'msg': f"Error semántico: variable ya declarada: '{id_tok['lexema']}'"})
        else:
            tipo_efectivo = tipo if valor_expr['tipo'] == tipo else 'error'
            scope_stack[-1][id_tok['lexema']] = {'tipo': tipo_efectivo, 'valor': valor_expr['valor'], 'entry': entry}
    ...
    def asignacion(ejecutar):
        id_tok = expect('IDENTIFICADOR', 'un identificador')
        expect('OPERADOR_ASIGNACION', "'='")
        valor_expr = expresion()
        expect('PUNTO_Y_COMA', "';'")

        info = declared(id_tok['lexema'])
        if info is None:
            errors.append({'line': id_tok['line'], 'msg': f"Error semántico: variable no declarada: '{id_tok['lexema']}'"})
            return

        if valor_expr['tipo'] != 'error' and valor_expr['tipo'] != info['tipo']:
            errors.append({'line': id_tok['line'],
                            'msg': f"Error semántico: no se puede asignar '{valor_expr['tipo']}' a '{info['tipo']}' (variable '{id_tok['lexema']}')"})
            return

        if valor_expr['tipo'] != 'error' and ejecutar:
            info['valor'] = valor_expr['valor']
            info['entry']['modificado'] = 'Sí'
    ...
    def sentencia(ejecutar=True):
        ...
        if t['lexema'] in TYPE_KEYWORDS and t['type'].startswith('PALABRA_RESERVADA:'):
            declaracion()
        elif t['type'] == 'IDENTIFICADOR' and peek_next()['type'] == 'OPERADOR_ASIGNACION':
            asignacion(ejecutar)
